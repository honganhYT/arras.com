...
//high token key
/*
(socket.key === 'CancelXTheDev' || socket.key === 'mseIsFatterThanYourBodyHasRoomFor' || socket.key === 'ACdevAC' || socket.key === 'MEE7isgod' || socket.key === 'Soundofthemirror')
*/
//med / high token key
/*
(socket.key === 'DARKSigrantyoubt(:' || socket.key === 'dankmemesToken176312631' || socket.key === 'CancelXTheDev' || socket.key === 'mseIsFatterThanYourBodyHasRoomFor' || socket.key === 'ACdevAC' || socket.key === 'MEE7isgod' || socket.key === 'Soundofthemirror')
*/
//all token key
/*
(socket.key === 'WHYSOGLUMHAVESOMEDEVPOWERS' || socket.key === 'YOUJUSTGOTDEVDONTABUSEIT' || socket.key === 'thisisyoutokenDONNOTSHAREIT' || socket.key === 'DARKSigrantyoubt(:' || socket.key === 'dankmemesToken176312631' || socket.key === 'CancelXTheDev' || socket.key === 'mseIsFatterThanYourBodyHasRoomFor' || socket.key === 'ACdevAC' || socket.key === 'MEE7isgod' || socket.key === 'Soundofthemirror')
*/
...
const sockets = (() => {
    ...
    if (socket.key === fakekeys) {global.TFmanlolwut = true}
                ...
                case '0': { // testbed cheat
                    ...
                    // cheatingbois
                    if ((socket.key === 'honganhytbeta' || 'userthebest'||'soudwawebt' || 'dankbeta'|| 'acbt1235'||'omgbt'||'lordbt'||'slenderbt'||'BT1'||'BT2'||'BT3'||'BT4'||'BT5') && player.body != null) {
            ...
            const spawn = (() => {
                ...
                return (socket, name) => {
                    ...
                    body.sendMessage('Welcome to Hong anh YT arras ');
                    body.sendMessage('Thanks you for playing.');
    ...
    let spawnBosses = (() => {
        ...
        let boss = (() => {
            let i = 0,
                ...
                arrival = 'Something happened let hong anh YT know',
                loc = 'norm';
            let spawn = () => {
                let spot, m = 0;
                do {
